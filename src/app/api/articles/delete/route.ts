...
// API-маршрут для скрытия статьи (установка is_published в false)
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Скрываем статью с ID: ${id}`);
    ...
    if (!result.success) {
      console.error("Все методы скрытия не удались:", result.error);
      return NextResponse.json(
...
async function standardUpdate(id: string) {
  try {
    ...
    if (error) {
      console.error("Ошибка при скрытии статьи (стандартный метод):", error);
...
async function serviceKeyUpdate(id: string) {
  try {
    ...
    if (error) {
      console.error("Ошибка при скрытии через сервисный ключ:", error);
      return { success: false, error: error.message };
    }
