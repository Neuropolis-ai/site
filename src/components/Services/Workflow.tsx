        ...
        <div>
            ...
            <div className={`w-full lg:w-[523px] p-[20px] rounded-xl border border-[#262626] overflow-hidden ${isDark ? 'process-card' : 'bg-gray-50 border-gray-200'}`}>
                ...
                >
                    ...
                    <div className="w-full overflow-hidden" style={{ mask: 'linear-gradient(to right, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 10%, rgb(0, 0, 0) 90%, rgba(0, 0, 0, 0) 100%)' }}>
                        <div className={`scroll-container-left workflow-row ${!isWorkflowVisible ? 'pause-animation' : ''}`}>
                            {/* First set of icons for seamless scrolling */}
                            <div className="scroll-content">
                                {Array(5).fill(0).map((_, setIndex) => (
                                    topRowIcons.map((icon, index) => renderIcon(icon, `top-${setIndex}-${index}`))
                                ))}
                            </div>
                            {/* Duplicated set for continuous loop */}
                            <div className="scroll-content">
                                {Array(5).fill(0).map((_, setIndex) => (
                                    topRowIcons.map((icon, index) => renderIcon(icon, `top-dup-${setIndex}-${index}`))
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full overflow-hidden" style={{ mask: 'linear-gradient(to right, rgba(0, 0, 0, 0) 0%, rgb(0, 0, 0) 10%, rgb(0, 0, 0) 90%, rgba(0, 0, 0, 0) 100%)' }}>
                        <div className={`scroll-container-right workflow-row ${!isWorkflowVisible ? 'pause-animation' : ''}`}>
                            {/* First set of icons for seamless scrolling */}
                            <div className="scroll-content">
                                {Array(5).fill(0).map((_, setIndex) => (
                                    bottomRowIcons.map((icon, index) => renderIcon(icon, `bottom-${setIndex}-${index}`))
                                ))}
                            </div>
                            {/* Duplicated set for continuous loop */}
                            <div className="scroll-content">
                                {Array(5).fill(0).map((_, setIndex) => (
                                    bottomRowIcons.map((icon, index) => renderIcon(icon, `bottom-dup-${setIndex}-${index}`))
                                ))}
                            </div>
                        </div>
                    </div>
